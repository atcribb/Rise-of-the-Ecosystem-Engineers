...
for(i in 1:nrow(infaunadom_df)){
  ...
  for(j in 1:iter){
    ...
    for(k in 1:length(subbed.ecoeng.formations)){
      ...
      #subsample formation data to 20 occs per formation
      ...
      this.formation.ecoengs.data <- subset(this.formation.subbed, genus %in% ecoeng_genera) #identify ecosysteme engineers 
...
save(infaunadom_df, file='bioturbator_dominance.RData')
