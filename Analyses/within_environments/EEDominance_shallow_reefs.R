...
shallow_reefs <- subset(all_reef_builders, bath=='shallow')


#=== get reef formations ===#
#save reef formations
reef_formations <- unique(shallow_reefs$formation)
#save reef genera 
reef_EE_genera <- unique(shallow_reefs$genus)
...
for(i in 1:nrow(reefdom_df)){
  ...
  this.stage.reef.data <- subset(shallow_reefs, stage==this.stage)
...
shallow_reefdom_df <- reefdom_df
save(shallow_reefdom_df, file='reefdominance_shallow_05-07-2024.RData')
