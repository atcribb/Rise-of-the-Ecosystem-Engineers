...
sili_reefs <- subset(all_reef_builders, lith=='siliciclastic')


#=== get reef formations ===#
#save reef formations
reef_formations <- unique(sili_reefs$formation)
#save reef genera 
reef_EE_genera <- unique(sili_reefs$genus)
...
for(i in 1:nrow(reefdom_df)){
  ...
  this.stage.reef.data <- subset(sili_reefs, stage==this.stage)
...
reefdom_sili_df <- reefdom_df
save(reefdom_sili_df, file='reefdominance_siliciclastic_05-07-2024.RData')
