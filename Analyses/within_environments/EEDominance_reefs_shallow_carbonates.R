...
shallow_carb_reefs <- subset(all_reef_builders, bath=='shallow' & lith=='carbonate')


#=== get reef formations ===#
#save reef formations
reef_formations <- unique(shallow_carb_reefs$formation)
#save reef genera 
reef_EE_genera <- unique(shallow_carb_reefs$genus)
...
for(i in 1:nrow(reefdom_df)){
  ...
  this.stage.reef.data <- subset(shallow_carb_reefs, stage==this.stage)
...
reefdom_shallow_carb_df <- reefdom_df
save(reefdom_shallow_carb_df, file='reefdominance_shallow_carbonate_05-07-2024.RData')
