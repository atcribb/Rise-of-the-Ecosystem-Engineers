...
shallow_sili_reefs <- subset(all_reef_builders, bath=='shallow' & lith=='siliciclastic')


#=== get reef formations ===#
#save reef formations
reef_formations <- unique(shallow_sili_reefs$formation)
#save reef genera 
reef_EE_genera <- unique(shallow_sili_reefs$genus)
...
for(i in 1:nrow(reefdom_df)){
  ...
  this.stage.reef.data <- subset(shallow_sili_reefs, stage==this.stage)
...
reefdom_shallow_siliciclastic_df <- reefdom_df
save(reefdom_shallow_siliciclastic_df, file='reefdominance_shallow_siliciclastic_05-07-2024.RData')
