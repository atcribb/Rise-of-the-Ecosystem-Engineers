...
infaunal_lifehabits <- c('deep infaunal', 'infaunal', 'semi-infaunal', 'semi-infaunal, solitary', 'shallow-infaunal',
                         ...
                         'shallow infaunal')
infauna_data <- subset(all_carb_data, life_habit %in% infaunal_lifehabits)
...
epifaunal_data <- subset(all_carb_data, life_habit=='epifaunal')
...
infauna_data <- subset(all_carb_data, genus %in% ecoeng_genera)
...
for(i in 1:nrow(infaunadom_df)){
  ...
  this.stage.data <- subset(all_carb_data, stage==this.stage)
...
infaunadom_carbs_df <- infaunadom_df
save(infaunadom_carbs_df, file='infauna_carbonate_dominance_05-07-2024.RData')
