...
all_shallow_carb_data <- subset(all_data, (bath=='shallow' & lith=='carbonate'))

phanero_stages <- stages$stage[4:95]
phanero_mids <- stages$mid[4:95]

#=== find fossil assemblages with and without ecosystem engineers ===#
table(all_shallow_carb_data$life_habit) #use life_habit to find infauna 
infaunal_lifehabits <- c('deep infaunal', 'infaunal', 'semi-infaunal', 'semi-infaunal, solitary', 'shallow-infaunal',
                         ...
                         'shallow infaunal')
infauna_data <- subset(all_shallow_carb_data, life_habit %in% infaunal_lifehabits)
...
epifaunal_data <- subset(all_shallow_carb_data, life_habit=='epifaunal')
...
infauna_data <- subset(all_shallow_carb_data, genus %in% ecoeng_genera)
...
for(i in 1:nrow(infaunadom_df)){
  ...
  this.stage.data <- subset(all_shallow_carb_data, stage==this.stage)
...
infaunadom_shallow_carb_df <- infaunadom_df
save(infaunadom_shallow_carb_df, file='infauna_shallow_carbonates_dominance_05-07-2024.RData')
