...
infaunal_lifehabits <- c('deep infaunal', 'infaunal', 'semi-infaunal', 'semi-infaunal, solitary', 'shallow-infaunal',
                         ...
                         'shallow infaunal')
infauna_data <- subset(all_deep_data, life_habit %in% infaunal_lifehabits)
...
epifaunal_data <- subset(all_deep_data, life_habit=='epifaunal')
...
infauna_data <- subset(all_deep_data, genus %in% ecoeng_genera)
...
for(i in 1:nrow(infaunadom_df)){
  ...
  this.stage.data <- subset(all_deep_data, stage==this.stage)
