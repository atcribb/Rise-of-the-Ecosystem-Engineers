...
deep_reefs <- subset(all_reef_builders, bath=='deep')


#=== get reef formations ===#
#save reef formations
reef_formations <- unique(deep_reefs$formation)
#save reef genera 
reef_EE_genera <- unique(deep_reefs$genus)
...
for(i in 1:nrow(reefdom_df)){
  ...
  this.stage.reef.data <- subset(deep_reefs, stage==this.stage)
