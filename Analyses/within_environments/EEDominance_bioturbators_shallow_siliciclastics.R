...
#subset only siliciclastic data
all_shallow_sili_data <- subset(all_data, (bath=='shallow' & lith=='siliciclastic'))

phanero_stages <- stages$stage[4:95]
phanero_mids <- stages$mid[4:95]

#=== find fossil assemblages with and without ecosystem engineers ===#
table(all_shallow_sili_data$life_habit) #use life_habit to find infauna 
infaunal_lifehabits <- c('deep infaunal', 'infaunal', 'semi-infaunal', 'semi-infaunal, solitary', 'shallow-infaunal',
                         ...
                         'shallow infaunal')
infauna_data <- subset(all_shallow_sili_data, life_habit %in% infaunal_lifehabits)
...
epifaunal_data <- subset(all_shallow_sili_data, life_habit=='epifaunal')
...
infauna_data <- subset(all_shallow_sili_data, genus %in% ecoeng_genera)
...
for(i in 1:nrow(infaunadom_df)){
  ...
  this.stage.data <- subset(all_shallow_sili_data, stage==this.stage)
  ...
  if(nrow(this.stage.infauna.data>1)){ #only proceed if there are enough formations 
    for(j in 1:iter){
...
infaunadom_shallow_sili_df <- infaunadom_df
save(infaunadom_shallow_sili_df, file='infauna_shallow_siliciclastic_dominance_05-07-2024.RData')
