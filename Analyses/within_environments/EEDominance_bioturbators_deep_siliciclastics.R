...
all_deep_sili_data <- subset(all_data, (bath=='deep' & lith=='siliciclastic'))

phanero_stages <- stages$stage[4:95]
phanero_mids <- stages$mid[4:95]

#=== find fossil assemblages with and without ecosystem engineers ===#
table(all_deep_sili_data$life_habit) #use life_habit to find infauna 
infaunal_lifehabits <- c('deep infaunal', 'infaunal', 'semi-infaunal', 'semi-infaunal, solitary', 'shallow-infaunal',
                         ...
                         'shallow infaunal')
infauna_data <- subset(all_deep_sili_data, life_habit %in% infaunal_lifehabits)
...
epifaunal_data <- subset(all_deep_sili_data, life_habit=='epifaunal')
...
infauna_data <- subset(all_deep_sili_data, genus %in% ecoeng_genera)
...
for(i in 1:nrow(infaunadom_df)){
  ...
  this.stage.data <- subset(all_deep_sili_data, stage==this.stage)
...
infaunadom_deep_siliciclastic_df <- infaunadom_df
save(infaunadom_deep_siliciclastic_df, file='infauna_deep_siliciclastic_dominance_05-07-2024.RData')
