...
carb_reefs <- subset(all_reef_builders, lith=='carbonate')


#=== get reef formations ===#
#save reef formations
reef_formations <- unique(carb_reefs$formation)
#save reef genera 
reef_EE_genera <- unique(carb_reefs$genus)
...
for(i in 1:nrow(reefdom_df)){
  ...
  this.stage.reef.data <- subset(carb_reefs, stage==this.stage)
...
reefdom_carbs_df <- reefdom_df
save(reefdom_carbs_df, file='reefdominance_carbonates_05-07-2024.RData')
